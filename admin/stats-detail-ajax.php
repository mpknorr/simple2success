...
if ($metric === 'resignups') {
    ...
    $sql = "SELECT timestamp AS signup_at,
                   leadid, name, email, country_detected, lang, source, paidstatus,
                   step1_at, username
...
echo json_encode(['rows' => $rows, 'metric' => $metric, 'page' => $page, 'total' => count($rows)]);
